...
class MCPServer:
    ...
    def handle_tools_list(self, request: MCPRequest) -> MCPResponse:
        ...
        tools_list = [
            tool.to_mcp_tool() for tool in self.tools.values()
        ]
    ...
    async def handle_tools_call(self, request: MCPRequest) -> MCPResponse:
        ...
        if not tool_name:
            return MCPResponse(id=request.id,
                               error=ErrorResponse(code=-32601, message="Missing required parameter: name"))

        if not self.tools.get(tool_name):
            return MCPResponse(id=request.id, error=ErrorResponse(code=-32601, message=f"Tool '{tool_name}' not found"))
        tool = self.tools[tool_name]

        try:
            result_text = await tool.execute(arguments)
            return MCPResponse(id=request.id, result=[{'type': 'text', 'text': result_text}])
        except Exception as tool_error:
            return MCPResponse(id=request.id, result=[
                {'type': 'text', "text": f"Tool execution error: {str(tool_error)}", 'isError': True}])
